...
function clothing_store_related_products_heading($translated_text, $text, $domain) {
...
function clothing_store_woocommerce_breadcrumb_separator($defaults) {
...
function clothing_store_custom_admin_notice() {
    ...
    if (!get_user_meta(get_current_user_id(), 'dismissed_admin_notice', true)) {
        ...
        if ($current_screen && $current_screen->id !== 'appearance_page_clothing-store-guide-page') {
            $theme = wp_get_theme();
            ?>
...
function clothing_store_enqueue_admin_script($hook) {
    ...
    wp_enqueue_style('clothing-store-dashboard-css', get_theme_file_uri('/inc/dashboard/dashboard.css'));
...
function clothing_store_custom_excerpt() {
    ...
    $word_limit = esc_attr(get_theme_mod('clothing_store_post_excerpt', '30'));
...
function clothing_store_copyright_link() {
    $footer_text = get_theme_mod('clothing_store_footer_text', esc_html__('Clothing Store WordPress Theme', 'clothing-store'));
    $credit_link = esc_url('https://www.ovationthemes.com/wordpress/free-clothing-store-wordpress-theme/');

    echo '<a href="' . $credit_link . '" target="_blank">' . esc_html($footer_text) . '<span class="footer-copyright">' . esc_html__(' By Ovation Themes', 'clothing-store') . '</span></a>';
}
